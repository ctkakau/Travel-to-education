...
###########################
testing <- sa_in_bigger(teu, 'Bay of Plenty Region')
testing$plot
```
...
# set colour palette
colPalette <- colorFactor("colPalette", domain = mini_df$Regional.Council)
...
leaflet(mini_df) %>%
  ...
  addCircles(lng = ~rc_centres$lon,
             lat = ~rc_centres$lat,
             fill = TRUE,
             stroke = TRUE,
             color = 'blue', #~colPalette(rc_centres$RC),
             weight = 1000) %>%
  addMiniMap(r_shp) %>%
  mapOptions() %>%
  addMeasure() %>%
...
```{r r5rIsochrone, include = TRUE, fig.cap="trying isochrones at SA3 level"}
# built from the r5r intro vignette
# https://cran.r-project.org/web/packages/r5r/vignettes/r5r.html

# # initialise Jave with rJava, check version = '21.0.5'
# rJava::.jinit()
...
```{r waitingToSeeIfImNeeded, include = TRUE, fig.cap='These are meant to be shapefiles and stuff'}
# # read in the sa3 shape file and filter to just the identifiers and geometry
# sa3Shp <- st_read(shp_sa3, quiet = TRUE)   %>%
#   select(c(1, 2, geometry)) %>%
#   rename(SA3 = 1,
#          SA3_chx = 2) %>%
#   filter(SA3_chx %in% unique(teu$SA3_chx[teu$Regional.Council %in% rc1]))
# 
# # summarise data to sa3, retain year, region, sa3, and mode of travel
# sa3_in_reg <- teu %>% 
#   filter(Regional.Council %in% rc1) %>%
#   group_by(Year, Regional.Council, SA3_chx, TravelMode, Value) %>%
#   summarise(Value = sum(Value))
#   
# ###########################
...
# 
# # one big plot highlighting the target regions
# reg_on_map <- function(dat, shp, reg){
#   
#   # regional shapefile
#   
#   # territorial shapefile
#   
#   # SA2? is there an SA3 shapefile?
#   
# }
# 
#   
# 
# # then create separate plots for each region
# # Tauranga
# # this creates a plot of the whole country with region shapes
# ggplot(data = teu %>% 
#          filter(Regional.Council %in% rc1) %>% 
...
#          summarise(total = sum(Value)),
#        )+
#   geom_sf(data = st_geometry(r_shp %>% filter(Regional.Council %in% rc1))) + # %>% filter(REGC2023_1 %in% reg1))
#   geom_sf(data = st_geometry(sa3Shp ), fill = 'blue') #%>% 
#   #           filter(r_shp[,2] %in% rc1))
#   
#   
