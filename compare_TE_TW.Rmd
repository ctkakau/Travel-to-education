...
ranked_travel_modes <- function(df, column, labels = c("education", "age")){
  ...
  bump <- ggplot(data = df,
    ...
    break_labs <- sort(unique(df$Location))
...
```{r fn_travel_3d}
# function to create 3d scatter plot to show total change, percent change, total count, by Location and travel mode
travel_3d <- function(df, labs = c("education", "all regions")) {
  ...
  df <- df |>
    ...
    mutate(
      difference = `2023`- `2018`,
      pct_change = (`2023`-`2018`)/`2018`,
      total = `2023`+`2018`
    )
  ...
  plt <- plot_ly(df, 
                 type = "scatter3d",
                 mode = "markers",
                 x = ~Location, y = ~pct_change, z = ~difference,
                 color = ~Travel_mode, 
                 size = ~total,
                 sizes=c(10, 100),
  ...
  plt <- plt %>% 
    layout(
      title= paste0("Travel to ", labs[1], ", ", labs[2],": percent and total change 2018-2023"),
      scene= list(
        xaxis = list(
          title= "Location",
...
if (!exists("two")) {
  ...
  abo <- ade_api_data_by_key(key = abo_key, resourceId = "CEN23_TRA_003")
  # rename columns for consistency
  names(abo) <- c("Age", "Gender", "Travel_mode", "Location", "Year", "Value") #AGE EDU GEN TED YEAR VALUE
  abo <- shorter_modes(abo, T)
}
```
...
Figure \@ref(fig:teeAgeRank) shows several changes in preference based on age and highlights shifts in preference between age 10 and 15 and again between age 15 and 20.  For ages below "15-19 years", the most preferred modes of travel to education are "Passenger", then "Walk or jog", followed by "School bus", "Bicycle".  The least preferred options in these age groups are "Private vehicle" and "Ferry".  For ages "15-19 years" the most preferred mode of travel to education is "Walk or jog" followed by "Passenger", "Public bus", driving a "Private vehicle", taking a "School bus", then "Train", "Bicycle", study "At home", "Other", and finally, "Ferry".  Driving a "Private vehicle" is the most preferred mode of transport for all remaining age groups, apart from "45-49 years" and "50-59 years", where it is second to study "At home".  
...
ranked_travel_modes(tee, Age, c("education", "age")) |> 
  htmlify()
```
For "20-24 years" age group the second most preferred mode is "Public bus" followed by "Walk or jog", study "At home", "Train", "Private vehicle", "Bicycle", "Other", "School bus", and finally "Ferry".  For age groups between "25-29 years" to "75-79 years" the top four modes of transport are consistent, with driving a "Private vehicle" as most preferred, then study "At home" (except the "45-50 years" and "50-54 years" age group), followed by "Public bus" and then "Walk or jog".  "Passenger" was the fifth option for all age groups from "30-34 years" to "75-79 years".  "Bicycle" is sixth most preferred option for "30-34 years" through to "70-74 years" age groups, except for the "45-49 years" where it is seventh, behind "Other".  


## Travel to work by Age  

Since the Travel to work category is restricted by age (i.e. 15 years and over) there is no data for age groups 0-4 years, 5-9 years, and 10-14 years.
...
The 3D plot in figure \@ref(fig:tee3D) provides an interactive view of the travel to education data by:  
  - using the colour of the marker to indicate the mode of travel
  - using the size of the marker to represent the combined count for years 2018 and 2023, for each mode of travel at each location
  - presenting locations along one axis
  - presenting the difference between years 2018 and 2023 along another axis
  - presenting the percentage change between years 2018 and 2023 along a third axis
  - changing the viewing perspective by zooming, panning, or rotating the plot.
  
Figure \@ref(fig:tee3D) shows counts for each mode of travel to education (size of circle), for each region, with one access showing the absolute change and another access showing the percentage change between 2018 and 2023.  The plot reinforces earlier analysis, showing the largest absolute increases between 2018 and 2023 occur for "Passenger" and study "At home", and the largest absolute decreases occur for "Walk or jog".  The largest percentage changes occur for "Ferry", "Train" and "Public bus", but in most regions, these modes have very low counts so even small absolute changes can result in high percentage changes.  The plot is dominated by high counts in the Auckland region and extreme percentage changes for "Train" and "Ferry" modes of travel, but allows the user to filter by mode. 

```{r tee3D, include=T, fig.cap="Travel to education for all regions: 3D interactive plot showing total count for mode of travel to education, by Location, difference, and percent change between 2018-2023"}
travel_3d(tee, c("education", "all regions"))
```

Similarly, figure \@ref(fig:two3D) presents the travel to work data for all regions, and reinforces the earlier findings that the largest absolute increases occur for work "At home", the largest absolute decrease was for "Train", "Public bus", "Walk or jog", and "Private vehicle" (driven mainly by decreases in Auckland)


```{r two3D, include=T, fig.cap="Travel to work for all regions: 3D interactive plot showing total count for mode of travel to education, by Location, difference, and percent change between 2018-2023"}
travel_3d(two, c("work", "all regions"))
```

## Analysis of Auckland Local Boards  

```{r akbGroups}
# grouped Auckland local boards
akb1 <- "aitem|owic|bert-"
akb2 <- "per H|bisc|ender|ara-P"
akb3 <- "kei|vonp|nkli|tiki"
akb4 <- "urew|Whau|gere-|kiekie"
akb5 <- "odn|paku|Puke|kere"
akb6 <- "aihe|eat "
```

The Auckland region holds a much higher population than any other region, nearly as much as the next three largest regions combined.  The Auckland region comprises several local boards, many of which, have populations comparable to some regions throughout the rest of the country.  This section conducts a brief analysis of the Auckland region and it's local boards.


```{r ake, include=T, fig.cap="Travel to education, Auckland local boards, all modes"}
# all Travel to work, Auckland
wrapped(abe, Location, F) |> 
  htmlify()
```

```{r rankedAbe, include = T, fig.cap="Preferred modes of travel to education: Auckland local boards"}
# preferred modes of travel, by Auckland local board
ranked_travel_modes(abe, Location, c("education", "Auckland local board")) |>
  htmlify()
```

```{r abe3D, include=T, fig.cap="Travel to education: 3D interactive plot showing total count for mode of travel to education, by Location, difference, and percent change between 2018-2023"}
travel_3d(abe, c("education", "Auckland local boards"))
```

```{r ako, include=T, fig.cap="Travel to work, Auckland local boards, all modes"}
# all Travel to work, Auckland
wrapped(abo, Location, F) |> 
  htmlify()
```



```{r rankedAbo, include = T, fig.cap="Preferred modes of travel to work: Auckland local boards"}
# preferred modes of travel, by Auckland local board
ranked_travel_modes(abo, Location, c("work", "Auckland local board")) |>
  htmlify()
```

```{r abo3D, include=T, fig.cap="Travel to work: 3D interactive plot showing total count for mode of travel to education, by Location, difference, and percent change between 2018-2023"}
travel_3d(abo, c("work", "Auckland local boards"))
```
...
```{r abe1, include=T, fig.cap="Travel to education: Mt Albert, Howick, Waitematā"}
# plots for Waitemata, Howick, Mt Albert
akb <- abe |> filter(str_detect(Location, akb1))
wrapped(akb, Location, F) |> htmlify()
wrapped(akb, Location) |> htmlify()
changes(akb, Location, F) |> htmlify()
changes(akb, Location, T) |> htmlify()
```

```{r abe2, include=T, fig.cap="Travel to education: Henderson, Hibiscus and Bays, Ōtara-Papatoetoe, Upper Harbour"}
# plots for Henderson...
akb <- abe |> filter(str_detect(Location, akb2))
wrapped(akb, Location, F) |> htmlify()
wrapped(akb, Location) |> htmlify()
changes(akb, Location, F) |> htmlify()
changes(akb, Location, T) |> htmlify()
...
```{r abe3, include=T, fig.cap="Travel to education: Devonport, Franklin, Kaipātiki, Ōrākei, "}
# plots for Devonport
akb <- abe |> filter(str_detect(Location, akb3))
wrapped(akb, Location, F) |> htmlify()
wrapped(akb, Location) |> htmlify()
changes(akb, Location, F) |> htmlify()
changes(akb, Location, T) |> htmlify()
...
```{r abe4, include=T, fig.cap="Travel to education: Māngere-Ōtāhuhu, Manurewa, Maungakiekie-Tāmaki, Whau"}
# plots for Mangere
akb <- abe |> filter(str_detect(Location, akb4))
wrapped(akb, Location, F) |> htmlify()
wrapped(akb, Location) |> htmlify()
changes(akb, Location, F) |> htmlify()
changes(akb, Location, T) |> htmlify()
...
```{r abe5, include=T, fig.cap="Travel to education: Papakura, Puketāpapa, Rodney, Waitākere"}
# plots for Papakura
akb <- abe |> filter(str_detect(Location, akb5))
wrapped(akb, Location, F) |> htmlify()
wrapped(akb, Location) |> htmlify()
changes(akb, Location, F) |> htmlify()
changes(akb, Location, T) |> htmlify()
...
```{r abe6, include=T, fig.cap="Travel to education: Waiheke, Aotea/Great Barrier"}
# plots for Mangere
akb <- abe |> filter(str_detect(Location, akb6))
wrapped(akb, Location, F) |> htmlify()
wrapped(akb, Location) |> htmlify()
changes(akb, Location, F) |> htmlify()
changes(akb, Location, T) |> htmlify()
...
```{r}
shapefile_query <- paste("https://datafinder.stats.govt.nz/services;key=", stats_shapefiles_key,  "/wfs/table-114916/?service=WFS&request=GetCapabilities", collapse="")
```
