...
ranked_by_age <- function(df, df_lab){
  ...
  bump <- ggplot(data = df) + 
    ...
    geom_point(aes(x = Age, y = Rank, colour = Travel_mode, size = Value))+
    labs(title = paste("Travel to",df_lab,"values by Age"),
         y = "Rank", x = "Age")
return(bump)
}
```
...
```{r teeAgeRank, include = T, fig.cap = "Ranked travel to education, by Age"}
ggplotly(ranked_by_age(tee, "education"))
```
...
```{r twoAgeRank, include = T, fig.cap = "Ranked travel to work, by Age"}
ggplotly(ranked_by_age(two, "work"))
```
