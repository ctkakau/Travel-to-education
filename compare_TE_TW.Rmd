...
#     colour = "grey50"
#   ) +
#   labs(title = paste("Travel to", df_lab, "by Age and Value"),
#        x = "Age", y = "Travel mode") + 
#   theme(legend.text = element_text("thousands"))
```
...
ggplotly(ranked_travel_modes(tee, Age, c("education", "age")))
```
...
ggplotly(ranked_travel_modes(two, Age, c("work", "age")))
```
```{r include=T, "Preferred modes of travel to education, by region"}
ggplotly(ranked_travel_modes(tee, Location, c("education", "region")))
```


```{r include=T, "Preferred modes of travel to work, by region"}
ggplotly(ranked_travel_modes(two, Location, c("work", "region")))
```
