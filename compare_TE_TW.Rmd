...
```{r teeRgnFct, include=T, fig.cap="Travel to education by Region"}
wrapped_regions(tee)
```
...
```{r teeChanges, include=T, fig.cap= "Travel to education absolute and percent changes 2018 to 2023"}
changes(tee|> filter(str_detect(Location, "anter|elling|aikat")), difference, F)
```
