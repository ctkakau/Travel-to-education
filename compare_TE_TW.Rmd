...
```{r fn_bundled_plots}
# function to create multiple plots for group of regions
bundled_plots <- function(df, bundle, column){
  p1 <- wrapped(df|> filter(str_detect(Location, {{bundle}})), {{column}}, F) 
  p2 <- wrapped(df|> filter(str_detect(Location, {{bundle}})), {{column}})  
  p3 <- changes(df|> filter(str_detect(Location, {{bundle}})), {{column}}, F) 
  p4 <- changes(df|> filter(str_detect(Location, {{bundle}})), {{column}}, T)
...
```{r include=T, fig.cap="Travel to work, Auckland, all modes"}
...
```{r, include = T, fig.cap="Travel to work, preferred modes of travel to work: Auckland local boards"}
# preferred modes of travel, by Auckland local boar
ranked_travel_modes(abo, Location, c("work", "Auckland local board")) |>
  htmlify()
```

```{r fn3Dplots, include=T, fig.cap="Travel to education: 3D plot"}
# assume grouping by Location
df <- tee |>
