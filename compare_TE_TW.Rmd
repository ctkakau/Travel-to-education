...
```{r}
df <- abo |>
  group_by(Location, Age, Travel_mode) |>
  summarise(Value = sum(Value, na.rm= T), .groups = "keep") |>
  arrange(Value)

ggplot(data = abo, 
       aes(x = Location, y = Age, colour = Travel_mode)
       ) +
  geom_point(
    aes(size = Value)
    ) +
...
akb2 <- "owic|bert-"
akb3 <- "per H|bisc|ender|ara-P"
akb4 <- "kei|vonp|nkli|tiki"
akb5 <- "urew|Whau|gere-|kiekie"
akb6 <- "odn|paku|Puke|Wait"
akb7 <- "aihe|eat "

wrapped(abo|> filter(str_detect(Location, akb1)), Location, F) |> 
htmlify()
...
```{r}
# wrapped(abo|> filter(str_detect(Location, rgn3)), Location, F) |> 
# +   htmlify()
# > wrapped(two|> filter(str_detect(Location, rgn3)), Location) |> 
# +   htmlify()
# > changes(two|> filter(str_detect(Location, rgn3)), Location, F)  |> 
